...
def _merge_ppf_accounts(items: list, account_number: str) -> dict | None:
    ...
    try:
        ...
        # Generate combined installment schedule
        today = date.today()
        installments = []
        running_balance = 0.0
        total_deposited = 0.0
        ...
        cumulative_interest = 0.0
        current_balance = 0.0
        last_compound_balance = 0.0
        months_since_compound = 0

        for m in range(1, tenure_months + 1):
            base_date = start_dt + relativedelta(months=m - 1)
            # For merged accounts, use 1st of each month as the reference date
            inst_date = base_date.replace(day=1) if base_date.day != 1 else base_date

            is_past = inst_date <= today

            # SIP deposit — check ALL phases active at this date (multiple can overlap)
            sip_deposit = 0.0
            for phase in combined_phases:
                p_start = datetime.strptime(phase["start"], "%Y-%m-%d").date()
                p_end_str = phase.get("end")
                p_end = datetime.strptime(p_end_str, "%Y-%m-%d").date() if p_end_str else None
                is_onetime = p_end and p_start == p_end

                if is_onetime:
                    # One-time deposit: match by year+month
                    if inst_date.year == p_start.year and inst_date.month == p_start.month:
                        sip_deposit += float(phase.get("amount", 0))
                    continue

                # Recurring SIP: check if inst_date falls within phase range
                if inst_date.year < p_start.year or (inst_date.year == p_start.year and inst_date.month < p_start.month):
                    continue
                if p_end and (inst_date.year > p_end.year or (inst_date.year == p_end.year and inst_date.month > p_end.month)):
                    continue

                p_amount = float(phase.get("amount", 0))
                p_freq = phase.get("frequency", "monthly")
                p_interval = _sip_freq_to_months(p_freq)
                months_since = (inst_date.year - p_start.year) * 12 + (inst_date.month - p_start.month)
                if months_since >= 0 and (months_since % p_interval == 0):
                    sip_deposit += p_amount
            ...
            # Annual compounding on March 31 (PPF financial year end)
            # Compound when this month is March and we have balance
            is_compound_month = (inst_date.month == 3 and m > 1)
            interest = 0.0
            if is_compound_month and running_balance > 0:
                interest = round(running_balance * rate_for_calc, 2)
                running_balance += interest
                cumulative_interest += interest
