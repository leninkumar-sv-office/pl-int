...
export default function MFImportPreviewModal({ data, onConfirm, onCancel }) {
  ...
  const { cas_id, statement_period, source, funds, summary } = data;
  ...
  // Group funds by AMC for display
  const amcGroups = {};
  funds.forEach(f => {
    const amc = f.amc || 'Unknown AMC';
    if (!amcGroups[amc]) amcGroups[amc] = [];
    amcGroups[amc].push(f);
  });
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '16px 20px' }}>
          {Object.entries(amcGroups).map(([amc, amcFunds], ai) => (
            <div key={ai} style={{ marginBottom: '20px' }}>
              ...
              {amcFunds.map((fund, fi) => (
                <div key={fi} style={{ marginBottom: fi < amcFunds.length - 1 ? '20px' : '0' }}>
                  {/* Fund section header */}
                  <div style={{
                    display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: '10px', color: 'var(--text-muted)', marginTop: '2px', fontFamily: 'monospace' }}>
                        ISIN: {fund.isin}
                        {fund.scheme_code && <> &nbsp;|&nbsp; Scheme: {fund.scheme_code}</>}
                    ...
                    <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>
                      {fund.transactions.length} txn{fund.transactions.length !== 1 ? 's' : ''}
                    </div>
                  </div>
