#!/bin/bash
# ═══════════════════════════════════════════════════════
#  Stock Portfolio Dashboard - Build & Run (Local Dev)
#  Builds frontend, activates venv, runs backend on :9998
# ═══════════════════════════════════════════════════════
...
# ── Step 3: Kill existing server on port 9998 ────────────
if lsof -ti:9998 &>/dev/null; then
    echo "Killing existing process on port 9998..."
    kill -9 $(lsof -ti:9998) 2>/dev/null
    sleep 1
fi

# ── Step 4: Run Backend ─────────────────────────────────
echo "── Starting Backend ────────────────────────────────"
echo "   Backend:  http://127.0.0.1:9998"
echo "   API Docs: http://127.0.0.1:9998/docs"
echo ""
...
cd "$BACKEND_DIR"
python3 -m uvicorn app.main:app --host 0.0.0.0 --port 9998 --reload
